...
func (p *Proxy) SessionCheck(s *driver.Session) error {
	// Max connection check.
	// max := 1000 //spanner.conf.Proxy.MaxConnections
...
// AuthCheck impl.
func (p *Proxy) AuthCheck(s *driver.Session) error {
	// Local login bypass.
	if localUserLogin(s) {
		return nil
	}
